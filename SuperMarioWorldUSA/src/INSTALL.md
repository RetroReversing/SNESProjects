...
These instructions explain how to set up the tools required to build **SuperMarioWorldUSA**, including [**rgbds**](https://github.com/gbdev/rgbds), which assembles the source files into a ROM.
...
WSL has its own file system that's not accessible from Windows, but Windows files *are* accessible from WSL. So you're going to want to install SuperMarioWorldUSA within Windows. You'll have to change the **current working directory** every time you open WSL.

For example, if you want to store SuperMarioWorldUSA in **C:\Users\\*\<user>*\Desktop**, enter this command:
...
Cygwin has its own file system that's within Windows, at **C:\cygwin64\home\\*\<user>***. If you don't want to store SuperMarioWorldUSA there, you'll have to change the **current working directory** every time you open Cygwin.

For example, if you want to store SuperMarioWorldUSA in **C:\Users\\*\<user>*\Desktop**:
...
To install the software required for **SuperMarioWorldUSA**:
...
To download the **SuperMarioWorldUSA** source files:

```bash
git clone https://github.com/yourGitRepo
cd SuperMarioWorldUSA/src
```

To build **SuperMarioWorldUSA.gb**:
